...
def split_2(v,Samples_upd , Samples_old):

		v_types = []
		v_count = dict()

		for i in xrange(len(Samples_upd)):
			if (Samples_upd[i][v] not in v_types):
				v_types.append(Samples_upd[i][v])
		...
		for i in xrange(len(Samples_upd)):
			# print len(Samples),i,v, Samples_upd[i] ,Samples_upd[i][v] 
			# print len(v_count) 
			v_count[Samples_upd[i][v]].append(Samples_old[i])					
...
def split_node(root):
	...
	MEDIAN = []
	Samples_upd = copy.deepcopy(Samples)
	v_count = 0
	for x in con_val :
		# print x , len(Samples[0])
		Samples = arr(Samples)
		median = np.median(Samples[:,x])
		Samples = np.ndarray.tolist(Samples)
		MEDIAN.append(median)
		...
		for i in xrange(len(Samples)):
			...
			if (Samples[i][x] > MEDIAN[v_count]):
				Samples_upd[i][x] = 1
			else :
				Samples_upd[i][x] = 0
		v_count += 1
		# print "LENEQUAL ",len(Samples_upd),len(Samples)

	# print MEDIAN
	v_index  = chooseBestVarToSplit(Samples_upd,AlreadySplit)
	if (v_index	==0):
		return root
	root.already_split.append(v_index)
	# print v_index
	if (v_index in con_val):
		v_types , v_count , s = split_2(v_index,Samples_upd,Samples)
		med_ind = 0
		for index in con_val:
			if (index ==v_index ):
				InternalNode.median = MEDIAN[med_ind]
				break
			med_ind += 1
	else:
	...
	# InternalNode.value = MEDIAN[v_index]
...
print root.variable , root.value
for child in root.children:
	print "		",child.variable , child.value
	for grandchild in child.children:
		print "			",grandchild.variable , grandchild.value
# print arr(root.data)[:,root.variable]
