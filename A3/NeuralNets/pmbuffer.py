...
input,Y = read_file(file_train,out)
...
input  = arr(input)
# input.reshape(len(input),86)

# input = [np.ones([len(input),1],dtype= int ),input]
print input
# exit(0)
...
# bias_parameters = [arr([0.0 for i in range(nrns_in_layer[j]*m )]) for j in range(len(nrns_in_layer)) ]
bias_parameters = [arr(np.ndarray.tolist(np.random.uniform(-1,1,nrns_in_layer[j]*m))) for j in range(len(nrns_in_layer)) ]
# bias_parameters.append(arr([0.0 for i in range(out*m)]))
...
for i in  range(len(bias_del_jtheeta)):
	print bias_del_jtheeta[i].shape
	# print del_jtheeta[i]*2.0

# print net

# NET_OUTPUT  = [net_output for j]
# net_output[0]  = np.dot(network_parameters[0],np.transpose(input))
...
#TRAIN TIME
#/////////////////#/////////////////#/////////////////#/////////////////#/////////////////#/////////////////#/////////////////#/////////////////
# del_jtheeta,redundant = initialize(nrns_in_layer,inp,out)
# del_jtheeta = arr(del_jtheeta)
# Batch Size
# r = 1
# Total Samples
m = len(input)
...
for epoch in range(EPOCHS):
	...
	for b in range(0,m/r):
		...
		for l in range(len(del_jtheeta_net)-1,-1,-1):
				...
				# 		for d in range(out):
				# 			del_jtheeta_net[l][p] += (Y[sample][d]-net_output[l][d])*net_output[l][d]*(1-net_output[l][d])
				...
				# 			del_jtheeta_net[l][p] += del_jtheeta_net[l+1][d]*net_output[l][p]*(1-net_output[l][p])*network_parameters[l+1][d][p]
			if (l==len(del_jtheeta_net)-1):
				del_jtheeta_net[l] = -1 * np.subtract(np.transpose(Y_label),net_output[l])*net_output[l]*(1-net_output[l])
			else:
				del_jtheeta_net[l] = np.dot(np.transpose(network_parameters[l+1]),del_jtheeta_net[l+1]) * net_output[l]*(1-net_output[l])
	...
	# 	# for each layer do
	# 	if (i==0):
	# 		net_output[i]  = np.dot(network_parameters[i],np.transpose(TEST_SAMPLES)) + bias_parameters[i]
	# 		net_output[i] = 1/(np.exp(net_output[i]*(-1)) + 1)
	# 	else:
...
def predict(TEST_SAMPLES,TEST_OUTPUT,net_output,net_parameters):
...
t0 = time.time()
predict(TEST_SAMPLES,TEST_OUTPUT,net_output,network_parameters)
t1 = time.time()
...
TEST_SAMPLES , TEST_OUTPUT = read_file(file_test,out)
t0 = time.time()
predict(TEST_SAMPLES,TEST_OUTPUT,net_output,network_parameters)
# print len(TE)
t1 = time.time()
...
# 					#// IF LAYER IS ZERO
# 					if (i==0):
# 						net += input[sample][k]*network_parameters[i][j][k] 
# 					else:
# 						net += net_output[i-1][k]*network_parameters[i][j][k]

# 				# ADD THE BIAS
# 				net += bias_parameters[i][j][0]
# 				# print type(net) 
